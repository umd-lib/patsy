...
field has a corresponding entry in the "identifier" field.

This script "expands" the rows, by creating one row per entry in the "hasPart"
field, matched to the corresponding entry in the "identifier" field.
...
|  pid     | displayTitle                                             | hasPart  | dmDate               | isMemberOfCollection | identifier   | dpExtRef                                                                                                            |
| -------- | -------------------------------------------------------- | -------- | -------------------- | -------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------- |
|umd:2100  | The great step forward : China women in the 20th century | umd:2101 | 2003-01-01T05:00:00Z | umd:1158             | FHgreatste02 | http://streamer.lib.umd.edu/ssdcms/i.do?u=d0c16b26570c476,http://streamer.lib.umd.edu/ssdcms/i.do?u=a74cdc7b162b40b |
|umd:2100  | The great step forward : China women in the 20th century | umd:2102 | 2003-01-01T05:00:00Z | umd:1158             | FHgreatste01 | http://streamer.lib.umd.edu/ssdcms/i.do?u=d0c16b26570c476,http://streamer.lib.umd.edu/ssdcms/i.do?u=a74cdc7b162b40b |
"""
...
def expand_parts(row):
    ...
    if len(parts) != len(ids):
        raise ValueError(f"pid: {row['pid']}: has_part len: {len(parts)} not equal to identifier len: {len(ids)})")

    for part, id in zip(parts, ids):
        new_row = row.copy()
        new_row["hasPart"] = part
        new_row["identifier"] = id
...
class FedoraAvExpandPartsTest(unittest.TestCase):
    def test_expand_parts_one_part(self):
        row = {"pid": "123", "field1": "one", "hasPart": "part1", "identifier": "id1"}

        expected_result = [
            {"pid": "123", "field1": "one", "hasPart": "part1", "identifier": "id1"}
        ]

        result = expand_parts(row)
        self.assertEqual(expected_result, result)

    def test_unequal_parts_lengths(self):
        # The number of items in "has_parts" is not the same as the number of
        # items in "identifier"
        row = {"pid": "123", "field1": "one", "hasPart": "part1,part2", "identifier": "id1"}

        with self.assertRaises(ValueError):
            expand_parts(row)

    def test_expand_parts_two_parts(self):
        row = {"pid": "123", "field1": "one", "hasPart": "part1,part2", "identifier": "id1,id2"}

        expected_result = [
            {"pid": "123", "field1": "one", "hasPart": "part1", "identifier": "id1"},
            {"pid": "123", "field1": "one", "hasPart": "part2", "identifier": "id2"}
        ]
    ...
    def test_expand_parts_three_parts(self):
        row = {"pid": "123", "field1": "one", "hasPart": "part1,part2,part3", "identifier": "id1,id2,id3"}

        expected_result = [
            {"pid": "123", "field1": "one", "hasPart": "part1", "identifier": "id1"},
            {"pid": "123", "field1": "one", "hasPart": "part2", "identifier": "id2"},
            {"pid": "123", "field1": "one", "hasPart": "part3", "identifier": "id3"}
        ]
