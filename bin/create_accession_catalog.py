...
def iter_space_delimited(dirlist):
    ptrn = r'^(\d{2}/\d{2}/\d{4}\s+\d{2}:\d{2}\s[AP]M)\s+([0-9,]+)\s(.+?)$'
    for n, line in enumerate(dirlist.lines, 1):
        ...
        if not match:
            ...
            yield Asset(filename=filename, bytes=bytes,
                        timestamp=timestamp, sourcefile=dirlist.filename,
                        sourceline=n, md5=None)


def iter_semicolon_delimited(dirlist):
    for n, line in enumerate(dirlist.lines, 1):
        cols = line.split(';')
        if cols[2] == 'Directory':
            continue
            ...
            yield Asset(filename=filename, bytes=bytes, timestamp=timestamp,     
                        sourcefile=dirlist.filename, sourceline=n, md5=None)

def iter_tabular_formats(self):
